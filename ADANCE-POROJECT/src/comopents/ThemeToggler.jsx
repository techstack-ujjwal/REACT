...
function ThemeToggleButton() {
  ...
  return (
    <button
      onClick={toggleTheme}
      className={`
        relative w-16 h-8 rounded-full transition-colors duration-300 cursor-pointer
    ...
    >
      <div
        className={`
          ...
          transition-transform duration-300 flex items-center justify-center text-xs shadow
...
function ThemedCard({ title, children }) {
  ...
  return (
    <div
      className={`
        p-6 rounded-lg shadow-md border transition-colors duration-300
        ${isDark ? "bg-gray-800/90 border-gray-700 text-white" : "bg-gray-50 border-gray-200 text-gray-800"}
      `}
    >
...
function ThemedButton({ children, variant = "primary", onClick }) {
  ...
  return (
    <button
      onClick={onClick}
      className={`px-4 py-2 rounded-lg font-medium transition-colors cursor-pointer shadow-sm ${getButtonClasses()}`}
    >
      {children}
    </button>
  );
